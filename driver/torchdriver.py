...
class PytorchOxModule(OxModule):
    ...
    def getTorchDataset(self):
        """
        convert neuralnet's DataFeed to a torch Dataset
        """
    ...
    def getTorchOptimizer(self):
        """
        convert neuralnet's Optimizer to a torch Optimizer
        """
    ...
    def getTorchLossTarget(self, streamMap, outMap):
        ...
        loss described by neuralnet's losses
        """
        lossValues = []
    ...
    def writeOnnx(self, dirname):
        ...
        if not self.neuralnetTest:
            data = iter(trainloader).next()
            self.writeOnnxModel(dirname,0, self.getStreamMap(data))

        # note for other frameworks. If neuralnetTest is always False, 
