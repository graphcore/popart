...
#define BOOST_TEST_MODULE AcoshOpPatternTest
...
BOOST_AUTO_TEST_CASE(AcoshOpPatternTest0) {
  ...
  ir.prepare({modelProto,
              ...
              *device,
              userOptions,
              Patterns({PreAliasPatternType::AcoshOpPattern})
                  .enableRuntimeAsserts(false)});

  BOOST_CHECK(ir.opsOfType(Onnx::Operators::Acosh_9).size() == 0);
...
BOOST_AUTO_TEST_CASE(AcoshOpPatternTest1) {
  ...
  BOOST_CHECK(noPatterns.isAcoshOpPatternEnabled() == false);

  Patterns defPatterns(PatternsLevel::Default);
  BOOST_CHECK(defPatterns.isAcoshOpPatternEnabled() == true);

  Patterns allPatterns(PatternsLevel::All);
  BOOST_CHECK(allPatterns.isAcoshOpPatternEnabled() == true);
}
