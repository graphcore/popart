...
def test_reducelogsum(op_tester):
    ...
    def init_builder(builder):
        ...
        for axes, keepdims in itertools.product(axes_list, keepdims_list):
            out = builder.aiOnnx.reducelogsum(
                [tensor],
                axes=axes,
                keepdims=keepdims,
                debugPrefix="test_reducelogsum_{0}_{1}".format(axes, keepdims))
    ...
    def reference(ref_data):
        ...
        for axes, keepdims in itertools.product(axes_list, keepdims_list):
            result.append(
                np.log(np.sum(data, axis=tuple(axes), keepdims=keepdims)))
        return result
...
def test_reducelogsum_training(op_tester):
    ...
    def reference(ref_data):
        ...
        for axes, keepdims in itertools.product(axes_list, keepdims_list):
            ...
            out = torch.log(torch.sum(tensor, dim=axes, keepdim=keepdims))
            result.append(out)
