...
def test_reducemax(op_tester):
    ...
    def init_builder(builder):
        ...
        for axes, keepdims in itertools.product(axes_list, keepdims_list):
            out = builder.aiOnnx.reducemax(
                [tensor],
                axes=axes,
                keepdims=keepdims,
                debugPrefix="test_reducemax_{0}_{1}".format(axes, keepdims))
    ...
    def reference(ref_data):
        result = []
        for axes, keepdims in itertools.product(axes_list, keepdims_list):
            result.append(np.max(data, axis=tuple(axes), keepdims=keepdims))
        return result
...
def test_reducemax_training(op_tester):
    ...
    def reference(ref_data):
        ...
        for axes, keepdims in itertools.product(axes_list, keepdims_list):
            ...
            for dim in sorted_axes:
                out = torch.max(out, dim=dim, keepdim=keepdims)[0]
            result.append(out)
