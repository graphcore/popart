...
def test_global_batch_size_correctness(tmpdir, reduction_type, loss_type,
    ...
    def model(compute_batch, batch_serialization_factor, accumulation_factor,
        ...
        d0 = builder.addInputTensor(
            popart.TensorInfo(
                'FLOAT', (compute_batch * batch_serialization_factor, 100)),
            'data0')
        data = {
            d0:
            input_data.reshape(
        ...
        if loss_type == "L1":
            ...
            l0 = builder.addInputTensor(
                ...
                'data0')
            data[l0] = np.random.randint(0, 100, size=batches_per_step * global_batch)\
    ...
    baseline_loss = np.mean(baseline_outputs[-1])
    ...
    for i, results in enumerate(tests):
        ...
        np.allclose(outputs[0].flatten(), baseline_outputs[0].flatten())

        loss = np.mean(outputs[-1])
        np.allclose(loss, baseline_loss)
