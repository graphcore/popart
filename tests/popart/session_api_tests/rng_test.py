...
def test_rng_set_and_get():
    ...
    def getModelWithRandomWeights():
        ...
        dShape = [2, 2]
        i0 = builder.addInputTensor(popart.TensorInfo("FLOAT16", dShape), "i0")
    ...
    np.random.seed(0)
    rng_state_interference = []
    tr_builder, out = getModelWithRandomWeights()
    ...
    # Input data
    data_a = np.random.rand(2, 2).astype(np.float16)
    ...
    def run_interference(interfering_session):
        ...
        rng_state_interference = interfering_session.getRNGState()
    ...
    # Step 3 -> interleaved training, validation, RNG not restaured
    session.weightsFromHost()
    session.setRNGState(init_rng)
    ...
    # There is a specific test case where the interference session doesn't modify the rng state
    # And the test fail without the check bellow:
    if rng_state_interference != rng:
