...
void ConstExprTest_Elementwise_Test(
    ...
    std::vector<T> in1,
    std::vector<T> output,
    std::function<TensorId(std::unique_ptr<poponnx::Builder> &builder,
...
BOOST_AUTO_TEST_CASE(ConstExprTest_Elementwise) {
  ConstExprTest_Elementwise_Test<int64_t>(
      ...
      {2, 3},
      {2, 4},
      [](std::unique_ptr<poponnx::Builder> &builder,
  ...
  ConstExprTest_Elementwise_Test<int64_t>(
    {4, 12}, {3, 8}, {1, 4}, {1, 2, 2},
    [](std::unique_ptr<poponnx::Builder>& builder, const std::vector<TensorId>&
