...
BOOST_AUTO_TEST_CASE(Builder_CustomOp_Into_WindowParameter_Op) {
  ...
  BOOST_CHECK_EQUAL(builder->getTensorShape(t0), std::vector<int64_t>{});
...
BOOST_AUTO_TEST_CASE(Builder_CustomOp_Into_Registered_Shape_Inference_Op) {
  // Build the model:
  //
  // in0 ----> Foo ----> t0 ---> Batchnorm --> o
  //       (Custom Op)
  //
  // Test that a tensor generated by a custom op (which lacks an onnx shape
  // inference method) can be an input to an op with a registered shape
  // inference function.
  ...
  // Verify that nop op has a shape inference function
  BOOST_CHECK(ShapeInferenceFunctions::hasFunction(
      Onnx::Operators::BatchNormalization_6));

  // Verify that the output tensor from the custom op node has been added
  // to the 'value info' field of the graph
  BOOST_CHECK(builder->hasValueInfo(t0));
