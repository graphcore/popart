...
def float32_to_float8_143(
    fval: float, truncate: bool = False, log2_scale: int = 0, exp_bias: int = 8
) -> int:
    ...
    Args:
        ...
        log2_scale  (int, optional): The input is scaled by pow(2, log2_scale) before
    ...
    # Overflow-then-inf, (1 + 7 / 8) * 2 ** ((2 ** 4 - 1) - exp_bias + log2_scale)
    # Where:
    # (1 + 7 / 8) is the max manissa value.
    # (2 ** 4 - 1) is the maximal exponent
    max_val = (1 + 7 / 8) * pow(2, ((2 ** 4 - 1) - exp_bias + log2_scale))
    if abs(fval) > max_val:
        return 0x80
...
def float32_to_float8_152(
    fval: float, truncate: bool = False, log2_scale: int = 0, exp_bias: int = 16
) -> int:
    ...
    # overflow-then-inf, 1.75 * (2 ** ((2 ** 5 -) 1 - exp_bias + log2_scale))
    # Where:
    # (1 + 3 / 4) is the max manissa value.
    # (2 ** 5 - 1) is the maximal exponent.
    max_val = (1 + 3 / 4) * pow(2, ((2 ** 5 - 1) - exp_bias + log2_scale))
    if abs(fval) > max_val:
        return 0x80
...
class TestFP8Conversion:
    ...
    def test_convertToFP8(
        ...
        Args:
            ...
            log2_scale (int): The input is scaled by pow(2, log2_scale) before
                conversion to ensure input falls in the representable fp8 range.
        ...
        if format_ == popart.DataType.FLOAT8_143:
            conv = np.array(
                [
                    float32_to_float8_143(i, True, log2_scale=log2_scale)
                    for i in a1.flatten()
                ]
            )
            ...
            conv = np.array(
                [
                    float32_to_float8_152(i, True, log2_scale=log2_scale)
                    for i in a1.flatten()
                ]
            )
            np.testing.assert_allclose(b1, conv, rtol=0.1, atol=1)
        ...
        c1 = self.conversion_func_to(dtype_to)(format_, b1, -log2_scale)
