...
class TestConv:
    ...
    def test_conv1d(self):
        ...
        weight = np.random.rand(out_channel, int(in_channel / group),
        ...
        torch_weight = torch.nn.Parameter(
            torch.tensor(weight).type(torch.float32))
        torch_outputs = nn.Conv1d(in_channel,
                                  out_channel, (kernel, ),
                                  strides,
                                  groups=group,
                                  bias=False)
    ...
    def test_conv3d(self):
        ...
        padding = (0, 0, 0, 0, 0, 0)
        dilation = (1, 1, 1)
        t = np.random.rand(batch_size, in_channel, in_deepth, height,
                           width).astype('float32')
        weight = np.random.rand(out_channel, int(in_channel / group), d_kernel,
                                h_kernel, w_kernel).astype('float32')
        ir = popxl.Ir()
        main = ir.main_graph
        with main:
            # host load
            ...
            input1 = popxl.h2d_stream([
                out_channel,
                int(in_channel / group), d_kernel, h_kernel, w_kernel
            ],
                                      popxl.float32,
                                      name="in_stream_1")
            b = ops.host_load(input1, "b")
        ...
        torch_weight = torch.nn.Parameter(
            torch.tensor(weight).type(torch.float32))
        torch_outputs = nn.Conv3d(in_channel,
                                  out_channel, (d_kernel, h_kernel, w_kernel),
                                  strides,
                                  groups=group,
                                  bias=False)
        torch_outputs.weight = torch_weight
    ...
    def test_conv2d_options(self):
        ...
        group = 1
        t = np.random.rand(batch_size, in_channel, height,
                           width).astype('float16')
        weight = np.random.rand(out_channel, int(in_channel / group), h_kernel,
                                w_kernel).astype('float16')
        ir = popxl.Ir()
        main = ir.main_graph
        with main:
            # host load
