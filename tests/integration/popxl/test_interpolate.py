...
class TestInterpolate:
    ...
    def test_interpolate_linear(self):
        ...
        with main:
            # host load
            input0 = popxl.h2d_stream(
                [in_channel, height, width], popxl.float32, name="in_stream_0"
            )
            a = ops.host_load(input0, "a")
            # custom interpolate.
    ...
    def test_interpolate_nearest_mode(self):
        ...
        with main:
            ...
            a = ops.host_load(input0, "a")
            # custom interpolate.
            o = ops.interpolate(a, scale_factor, "nearest", "round_prefer_ceil")
            # host store
            o_d2h = popxl.d2h_stream(o.shape, o.dtype, name="out_stream")
            ops.host_store(o_d2h, o)
        # get the result
        ...
        np.testing.assert_allclose(
            out_onnx, list(outputs.values())[0], rtol=1e-7, atol=1e-7
        )
