...
def test_full_recompute_pipelining(explicit):
    ...
    def run_test(mode=None, verify=None, explicit=False):
        ...
        mask = builder.addInputTensor(popart.TensorInfo("FLOAT", mask_shape), "mask")
        ...
        dataFlow = popart.DataFlow(
            batches_per_step,
            {
                ...
                popart.reservedGradientPrefix() + qkv_1: popart.AnchorReturnType("All"),
                popart.reservedGradientPrefix() + qkv_2: popart.AnchorReturnType("All"),
                popart.reservedGradientPrefix() + qkv_3: popart.AnchorReturnType("All"),
            },
        )
        ...
        with tu.create_test_device(numIpus=3, opts={"compileIPUCode": False}) as device:
    ...
    p_anchors = run_test(popart.RecomputationType.Pipeline, verify, explicit=explicit)
...
def test_implicit_pipelining_custom_fwd_only():
    ...
    def run(inference_during_training):
        ...
        dataFlow = popart.DataFlow(
            batches_per_step,
            {o: popart.AnchorReturnType("All"), l1: popart.AnchorReturnType("All")},
        )

        infDataFlow = popart.DataFlow(
            batches_per_step * accumulation_factor,
            {o: popart.AnchorReturnType("All"), l1: popart.AnchorReturnType("All")},
        )
        ...
        with tu.create_test_device(
            numIpus=3, opts={"compileIPUCode": False}
        ) as device0:
            session = popart.TrainingSession(
                fnModel=proto,
                dataFlow=dataFlow,
            ...
            if inference_during_training:
                ...
                with tu.create_test_device(
                    ...
                    inf_session = popart.InferenceSession(
                        fnModel=proto,
...
def test_implicit_pipelining_custom_fwd_only_no_copy():
    ...
    with tu.create_test_device(numIpus=3, opts={"compileIPUCode": False}) as device0:
        session = popart.TrainingSession(
            fnModel=proto,
            dataFlow=dataFlow,
            userOptions=opts,
        ...
        copies = [op for graph in ir for op in ir[graph] if (op["type"] == "IpuCopy")]
