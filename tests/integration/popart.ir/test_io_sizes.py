...
class TestIOSizes():
    def test_uneven_host_loads(self):
        ...
        with main:
            ...
            d0_h2d = pir.h2d_stream((DATA_SHAPE, DATA_SHAPE),
                                    pir.float32,
                                    name="d0_stream")
            e0_h2d = pir.h2d_stream((DATA_SHAPE, DATA_SHAPE),
                                    pir.float32,
                                    name="e0_stream")
            out_d2h = pir.d2h_stream((DATA_SHAPE, DATA_SHAPE),
                                     pir.float32,
                                     name="out_d2h_stream")
            with repeat_sg, pir.in_sequence():
    ...
    def test_io_sizes(self, monkeypatch, data_size: Tuple[int],
        ...
        with main:
            ...
            out_d2h = pir.d2h_stream(data_size,
                                     pir.float32,
                                     name="out_d2h_stream")
            with repeat_sg, pir.in_sequence():
