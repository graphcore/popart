...
def _create_model_with_sparse_constant():
    ...
    sparse_tensor = onnx.helper.make_sparse_tensor(data_tensor, indices_tensor, [8])

    const_node = onnx.helper.make_node(
        "Constant", [], ["ConstOut"], sparse_value=sparse_tensor
    )
    identity_node = onnx.helper.make_node("Identity", ["ConstOut"], ["IdentityOut"])

    out_info = onnx.helper.make_tensor_value_info(
        "IdentityOut", onnx.helper.TensorProto.FLOAT, [4]
    )
    graph_def = onnx.helper.make_graph(
        [const_node, identity_node], "test-model", [], [out_info]
    )
...
def test_sparse_data():
    ...
    anchors = {
        output.name: popart.AnchorReturnType("All") for output in model.graph.output
    }
