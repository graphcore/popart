...
def run_sgd_mixed_mode(steps, opt_dicts, enable_outlining, tmpdir, dtype=np.float32):
    ...
    for i, opt_dict in enumerate(opt_dicts):
        ...
        for j in range(len(gt_onnx.graph.initializer)):
            ...
            val = to_array(val)
            # print(gt, val)
            assert np.allclose(gt, val, rtol=1.0e-2, atol=1.0e-3)
...
def test_sgd_mixed_mode_0(tmpdir, sgdAccMm, accumType, accl1Type):
    ...
    if sgdAccMm == popart.SGDAccumulatorAndMomentum.Combined:
        ...
        if accumType != popart.DataType.FLOAT or accl1Type != popart.DataType.FLOAT:
            return
...
def test_sgd_mixed_mode_1(tmpdir):
    ...
    sgd00.insertSpecific(
        "w_0", {"momentum": (0.9, True), "velocityScaling": (0.25, True)}
    )
    sgd00.insertSpecific(
        "b_0", {"momentum": (0.9, True), "velocityScaling": (0.25, True)}
    )
    ...
    sgd10.insertSpecific(
        "w_0", {"momentum": (0.9, False), "velocityScaling": (0.25, False)}
    )
    sgd10.insertSpecific(
        "b_0", {"momentum": (0.9, False), "velocityScaling": (0.25, False)}
    )
    ...
    sgd20.insertSpecific(
        "w_0", {"momentum": (0.9, False), "velocityScaling": (0.25, True)}
    )
    sgd20.insertSpecific(
        "b_0", {"momentum": (0.9, False), "velocityScaling": (0.25, True)}
    )
    ...
    optMaps = [{0: sgd00, 5: sgd05}, {0: sgd10, 5: sgd15}, {0: sgd20, 5: sgd25}]
