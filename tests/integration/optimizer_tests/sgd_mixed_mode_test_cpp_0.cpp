...
BOOST_AUTO_TEST_CASE(SgdMixedModeTest0) {
  ...
  // ------------
  {
    ...
    opt0.insertSpecific(
        w1name,
        {{"learningRate", {0.2, false}}, {"weightDecay", {0.0f, true}}});
  ...
  // non-const
  {
    ...
    auto opt0 = SGD({{"defaultLearningRate", {2, true}},
                     {"defaultWeightDecay", {0, true}},
                     {"lossScaling", {20, false}}});

    opt0.insertSpecific(
        w1name,
        {{"learningRate", {4.0, true}}, {"weightDecay", {0.125, true}}});
    ...
    Expectation e0 = {
        100 - 2.0 - 2.0,
        false, // scaled learning rate is not constant, because loss scaling
        true   // isn't weight decay scaling factor is constant
    };
    ...
    //
    // weight 1 uses local optimizer values
    Expectation e1 = {44, false, true};

    Expectation e2 = {300 - 2.0 - 2.0, false, true};
