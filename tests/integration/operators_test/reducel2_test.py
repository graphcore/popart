        ...
        for axes, keepdims in itertools.product(axes_list, keepdims_list):
            if axes is USE_DEFAULT_AXES:
                out = builder.aiOnnx.reducel2(
                    [tensor],
                    keepdims=keepdims,
                    debugContext="test_reducel2_default_{0}".format(keepdims),
                )
            else:
                out = builder.aiOnnx.reducel2(
                    [tensor],
                    axes=axes,
                    keepdims=keepdims,
                    debugContext="test_reducel2_{0}_{1}".format(axes, keepdims),
                )
...
def test_reducel2_training(op_tester):
        ...
        sum = builder.aiOnnx.sum(
            [
                builder.aiOnnx.reducesum(
                    [r], axes=axes, keepdims=False, debugContext="test_reducel2_all"
                )
    ...
    def reference(ref_data):
        ...
        for axes, keepdims in itertools.product(axes_list, keepdims_list):
            ...
            out = torch.sqrt(torch.sum(tensor ** 2, dim=dim, keepdim=keepdims))
            result.append(out)
