...
def test_enable_conv_dithering_error(capfd, apply_to_conv, dithering,
                                     exception_string):
    if exception_string is not None and isinstance(exception_string, str):
        with pytest.raises(popart.popart_exception) as e_info:
            conv_enable_dithering(capfd,
                                  apply_to_conv=apply_to_conv,
                                  dithering=dithering)
        assert e_info.value.args[0] == exception_string
    else:
        output = conv_enable_dithering(capfd,
                                       apply_to_conv=apply_to_conv,
                                       dithering=dithering)
        matches = re.findall(" +enableConvDithering +" + str(int(dithering)),
                             output)
