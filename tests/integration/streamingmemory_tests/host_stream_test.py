...
def create_model_pipelined(
    bufferStreams: bool = False, pipelining: bool = False, device=None
) -> Dict:
    ...
    stepio = popart.PyStepIO({ip: trainingData, lb: trainingDataLables}, anchors)

    return {"session": session, "stepio": stepio, "anchors": anchors, "out": sm}


def check_ops(
    session: popart.TrainingSession, bufferStreams: bool, numOps: int
) -> None:
...
def test_basic_host_load_output(enable_pipelining: bool):
    ...
    with tu.create_test_device(numIPUs) as d1, tu.create_test_device(numIPUs) as d2:
        bundle_false = create_model_pipelined(
            bufferStreams=False, pipelining=enable_pipelining, device=d1
        )
        bundle_true = create_model_pipelined(
            bufferStreams=True, pipelining=enable_pipelining, device=d2
        )
        ...
        for step in range(5):
            ...
            assert np.allclose(
                bundle_false["anchors"][bundle_false["out"]],
...
def get_model(
    ...
    builder.addOutputTensor(sm)
    o = builder.aiGraphcore.nllloss([sm, lb], reduction=popart.ReductionType.Mean)

    art = popart.AnchorReturnType("All")
    data_flow = popart.DataFlow(
        batches_per_step, {ip: art, lb: art, o: art, sm: art, a: art, relu: art}
    )
...
def run_model(
    ...
    stepio = popart.PyStepIO(
        {"main_input_123": in_array, "label_input_456": label_array}, anchors
    )
    ...
    with tu.create_test_device(ipus) as d1, tu.create_test_device(ipus) as d2:
        ...
        for step in range(steps):
            ...
            label_array = np.random.randint(
                low=0, high=(channels * data_len * data_len), size=label_shape_true
            ).astype(np.int32)
            # Only provide one session.run's worth of data.
            tuple_1 = run_model(sesstion_true, anchors_true, in_array, label_array)

            tuple_2 = run_model(session_false, anchors_false, in_array, label_array)
