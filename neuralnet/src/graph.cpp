...
template <class T>
void appendSequence(std::stringstream & ss, T t){
  int index = 0;
  ss << '[';
  for (auto & x : t){
    if (index != 0){
...
void Op::connectInTensor(InIndex inIndex, TensorId tenId) {
  ...
  ptensor->consumers.push_back(this);
}
...
void TensorIndexMap::append(std::stringstream &ss){
  ...
  for (auto &index_tensor : tensor_map) {
    ...
    ss << '@' << index_tensor.first << ':' << index_tensor.second->name << ':' 
...
Graph::Graph(onnx::ModelProto &&inMod,
  ...
  // split conv with bias into two ops
  splitConvBias();
...
void Graph::removePadSizeZero(){
  for (auto & op : opsOfType(OpType::PAD)){
    if (!isLogged(op->input.tensor(0)->name)){
      remove(op->input.tensor(0)->name, op->opId);
    }
  }
}
...
// remove []->() where [] is Tensor and () is Op
void Graph::remove(TensorId name, OpId opId){
...
const std::vector<std::string> & VectorAndSet::v(){
  return v_vals;
}
...
void Graph::addActivationTensor(TensorId tenId){
  tensors[tenId] = std::unique_ptr<Tensor>(
      new Tensor(tenId, TensorType::Activation, this));
}
...
const Node * Op::getNode(){
  if (!fromNode()){
    throw error("Op not from pointer");
  }
  return ptrNode;
}
...
Op::Op(OpId id, const onnx::NodeProto &node, Graph *pg)
    : opId(id), op_type(node.op_type()),
      opType(pg->opTypes.get(op_type)), domain(node.domain()), pgraph(pg) {}

std::unique_ptr<Op>
Graph::addOp(OpId opId, const onnx::NodeProto &node) {
