...
def _worker_loop(dataset, index_queue, data_queue, done_event, collate_fn,
    ...
    try:
        ...
        # Reduce the priority of the work thread, so that the main thread runs first
        os.nice(5)
            ...
            try:
                if log_statisics:
                    ...
                    if len(processing_times) > 8:
                        poponnx.getLogger().info(
                            "DataLoader worker:{0}  waiting: {1:6.4f} processing:{2:6.4f}"
                            .format(
                                worker_id,
...
def default_collate(batch, tensor_type=None):
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        # Due to ' _th_cat is not implemented for type torch.HalfTensor' we can not
        # add a transform that created batches of HalfTensor's so for not we will
        # use an option
        if tensor_type != None:
            return torch.stack(
...
class _DataLoaderIter(object):
    ...
    def __init__(self, loader):
        ...
        if self.num_workers > 0:
            ...
            # Set to try when resetting the iterator and we want to consume any outstanding
            # data from the workers
    ...
    def reset(self):
        ...
        poponnx.getLogger().debug("Resetting the dataloaderiterator")
...
class DataLoader(object):
  ...
  This is a customized DataLoader for poponnx which is inspired by the torch
  DataLoader. The difference being that the __iter__ call reuses the _DataLoader
  when called multiple times, to prevent the work processes being stopped and 
  respawned.
  ...
  Arguments:
      ...
      tensor_type : The type of tensor to be returned. By default the type will be float32.
          This value can be set to 'float32','float16' to return the desired type
  ...
  """
    ...
    def __init__(self,
        ...
        poponnx.getLogger().info(
            "DataLoader created batchsize:{} num_workers:{} shuffle:{} tensor_type:{}"
    ...
    def __iter__(self):
        ...
        # Only create the iterator once, as will fork process when created which
        # causes problems if the current process already has synchronisation objects
        if self.dataloaderiter is None:
            self.dataloaderiter = _DataLoaderIter(self)

        # Reset the data loader iterator
        self.dataloaderiter.reset()
