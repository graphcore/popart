...
def remote_store(remote_buffer: RemoteBuffer, offset: Union[int, Tensor],
    ...
    This op is typically used to store different, identically-shaped
    tensors to the same remote buffer by specifying the offset.

    Instances of the op with matching ``remote_buffer_id`` (specified in ``remote_buffer``)
    ...
    and ``dtype`` because ``shape`` and ``dtype`` needs to be fixed for each ``remote_buffer_id``.
