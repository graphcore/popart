...
def roi_align(t: Tensor, rois: Tensor, batch_index: Tensor,
    ...
    Args:
        t (Tensor):
            Input data tensor from the previous operator; 4-D feature map of shape (N, C, H, W), where N is the batch size,
            C is the number of channels, and H and W are the height and the width of the data.
        rois (Tensor):
            RoIs (Regions of Interest) to pool over; rois is 2-D input of shape (num_rois, 4) given as [[x1, y1, x2, y2], ...].
            The RoIs' coordinates are in the coordinate system of the input image. Each coordinate set has a 1:1 correspondence
            with 'batch_index' input.
        batch_index (Tensor):
            1-D tensor of shape (numRois,) with each element denoting the index of the corresponding image in the batch.
        output_size (Tuple[int]):
            Pooled output height and width.
        spatial_scale (float):
            Multiplicative spatial scale factor to translate ROI coordinates from their input spatial scale to the scale used when
            pooling, i.e., spatial scale of the input feature map `t` relative to the input image.
        sampling_ratio (int):
            Number of sampling points in the interpolation grid used to compute the output value of each pooled output bin.
    Returns:
        out (Tensor):
            RoI pooled output Y, 4-D tensor of shape (numRois, channels, aligned_height_, aligned_width_). The r-th batch element
            Y[r-1] is a pooled feature map corresponding to the r-th RoI t[r-1].
    """
