...
weights = np.random.rand(chans_out, chans_in, kernel_size,
...
leaky_relu = builder.customOp(opName="LeakyRelu",
                              opVersion=1,
                              domain="custom.ops",
                              inputs=[x],
                              attributes={})[0]
...
myinput = 10 * np.random.rand(batch_size, chans_in, size, size).astype(
    np.float32)
