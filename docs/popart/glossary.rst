...
This chapter defines some of the specialised terminology that is used in the PopART documentation. You can find a full list in :doc:`glossary:index`.
...
.. glossary::
  ...
  Virtual graph
    A subdivision of a graph to a subset of IPU tiles. While ``virtualGraphId`` in PopART refers to the graph associated with an IPU, the virtual graph can be subdivided further into tile sets ``IO`` and ``Compute``.
  ...
  Replicated tensor sharding
    ...
    Increases performance, especially in conjunction with :term:`Streaming Memory` and decreases on-chip memory usage.
