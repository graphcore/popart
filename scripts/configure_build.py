...
class CMakeScript:
    ...
    def setup_colors(self):
        ...
        try:
            ...
            for color in colors:
                ...
                curses.init_pair(WHITE_BG + color, color,
                                 curses.COLOR_WHITE)  # setup color pairs
            curses.init_pair(curses.COLOR_GREEN, curses.COLOR_GREEN, -1)
            curses.init_pair(curses.COLOR_YELLOW, curses.COLOR_YELLOW, -1)
            curses.init_pair(curses.COLOR_GREEN + WHITE_BG, curses.COLOR_GREEN,
                             curses.COLOR_WHITE)
            curses.init_pair(curses.COLOR_YELLOW + WHITE_BG,
                             curses.COLOR_YELLOW, curses.COLOR_WHITE)
            # setup black on white color pair
            curses.init_pair(BLACK_ON_WHITE, curses.COLOR_BLACK,
                             curses.COLOR_WHITE)
    ...
    def create_options_dict(self) -> None:
        ...
        self.add_cmake_arg("INTERNAL_RELEASE", "", ["OFF", "ON"], 1,
                           "Graphcore_target_access", False)
        self.add_cmake_arg(
            "POPART_USE_STACKTRACE",
            'Enable boost stacktrace reports in error messages', ["OFF", "ON"],
            1, "PopART")
        self.add_cmake_arg("POPART_BUILD_TESTING", "Build the popart tests",
                           ["OFF", "ON"], 1, "PopART")
        self.add_cmake_arg(
        ...
        self.add_cmake_arg(
            ...
            " containers of pointers", ["OFF", "ON"], 0, "PopART")
        self.add_cmake_arg(
        ...
        self.add_cmake_arg("CMAKE_BUILD_TYPE",
                           "Changes how PopART is built (Release/Debug)",
                           ["Release", "Debug"], 0, "PopART")
        self.add_cmake_arg(
            "CMAKE_BUILD_TYPE",
            "Changes how the rest of the view is built (Release/Debug)",
            ["Release", "Debug"], 0, "root")
        self.add_cmake_arg("USE_LOGS", "Record build results in logfiles",
                           ["OFF", "ON"], 1, "root")
        self.add_cmake_arg("BUILD_DOCS", "Build Documentation", ["OFF", "ON"],
                           0, "DOCS_POPLAR")
        self.add_file_arg(
            "SERVER_COOKIE_FILE",
            "Absolute path to file containing authorisation cookie for the elasticsearch server.",
            "", "PopART")
        self.add_string_arg("SWDB_PACKAGE_NAME", "SWDB Package Name", "",
                            "PopART")
    ...
    def get_chosen_value(self, name: str, location: str = "root") -> str:
        ...
        return self.options[location][name]["states"][self.state[location]
                                                      [name]]
    ...
    def draw_chosen_value(self, name: str, states: dict, height: int, i: int,
        ...
        if states["type"] in ["str", "file"]:
            ...
            is_default_option = states["states"].index(
        ...
        if self.has_colors:
            ...
            if i == self.cursor:
                ...
                self.stdscr.addstr(height, 50, selected,
                                   curses.color_pair(curses.COLOR_WHITE))

    def draw_name_desc_cursor(self, name: str, i: int, height: int, rows: int,
                              cols: int, states: dict) -> None:
        ...
        if self.has_colors:
            ...
            if i == self.cursor:
                ...
                self.stdscr.addstr(height, 0, name,
                                   curses.color_pair(curses.COLOR_WHITE))
        if color == BLACK_ON_WHITE:
            self.stdscr.addstr(height, len(name),
                               "\u2588" * (cols - len(name)),
                               curses.color_pair(curses.COLOR_WHITE))
            self.stdscr.addstr(rows - 2, 0, states["desc"],
                               curses.color_pair(curses.COLOR_WHITE))
            ...
            for i in self.options.keys():
                ...
                if i != "root":
                    self.draw_name_desc_cursor(
                        f"{i.replace('_', ' ').lower()} options", offset,
                        height, rows, cols,
                        {"desc": f"Options for {i.lower()}"})
                    modified = False
                    for option in self.options[i].keys():
                        if self.state[i][option] != self.options[i][option][
                                "default"]:
                            modified = True
                            break
                    effects = 0
                    if modified:
                        ...
                        self.stdscr.addstr(height, 50, "Modified",
                                           curses.color_pair(color) + effects)
                    offset += 1
        return offset
    ...
    def handle_scroll(self, rows: int) -> None:
        ...
        self.scroll = max(self.cursor + 5 - rows, min(self.scroll,
                                                      self.cursor))
    ...
    def handle_keypress(self, key: int, rows: int) -> Union[str, None]:
        ...
        if key == curses.KEY_DOWN:
            ...
            if rows + self.scroll - self.cursor < 5:
                ...
                self.scroll = min(self.cursor - rows + 4,
                                  self.num_options + 3 - rows)
        ...
        if key == ENTER or key == curses.KEY_RIGHT or chr(key) == " ":
            ...
            if self.options[self.loc()][name]["type"] == "str":
                ...
                file_select = FileSelectMenu(
                    ...
                    allow_folder=self.options[
                        self.loc()][name]["allow_folder"])
                ...
                if self.state[self.loc()][name] > len(
                        self.options[self.loc()][name]["states"]) - 1:
    ...
    def generate_command(self) -> str:
        ...
        for loc in self.state.keys():
            for name in self.state[loc].keys():
                ...
                if self.state[loc][name] != self.options[loc][name][
                        "default"] or not self.options[loc][name]["omit"]:
...
class FileSelectMenu:
    ...
    def render(self, rows: int, cols: int):
        ...
        """
        self.stdscr.erase()
        self.stdscr.addstr(0, 0, self.path,
                           curses.COLOR_WHITE + curses.A_UNDERLINE)
        for i, v in enumerate(self.dirs + self.files):
            ...
            if i == self.cursor:
                ...
                self.stdscr.addstr(i + 1 - self.scroll, 0, "\u2588" * cols,
                                   curses.COLOR_WHITE)
            self.stdscr.addstr(i + 1 - self.scroll, 0, v,
                               curses.COLOR_WHITE + effect)
    ...
    def handle_keypress(self, key: int, rows: int) -> Union[str, int, None]:
        ...
        if key == curses.KEY_DOWN:
            ...
            if rows + self.scroll - self.cursor < 6:
                # it is 5 because it is 5 rows from the bottom of the terminal
                self.scroll = min(self.cursor - rows + 5,
                                  self.num_options + 4 - rows)
...
@pytest.mark.parametrize("statechanges,result", [
    ({},
