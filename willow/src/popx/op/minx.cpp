...
MinGradOpx::MinGradOpx(Op *op_, Devicex *devicex_) : Opx(op_, devicex_) {}

void MinGradOpx::grow(poplar::program::Sequence &prog) const {
  ...
  auto mask =
      popops::map(graph(),
                  ...
                  {get(inId(MinGradOp::getFwdOutInIndex())),
                   get(inId(MinGradOp::getFwdInIndex()))},
                  prog,
                  idStr());
  ...
  auto result = popops::map(graph(),
                            pe::Mul(pe::_1, pe::_2),
                            {mask, get(inId(MinGradOp::getGradInIndex()))},
                            prog,
                            idStr());

  auto shapeOfOutputOfFwdOp = inInfo(MinGradOp::getFwdOutInIndex()).shape();
  auto shapeOfInputToFwdOp  = inInfo(MinGradOp::getFwdInIndex()).shape();
  ...
  insert(outId(MinGradOp::getOutIndex()),
         out.reshape(outInfo(MinGradOp::getOutIndex()).shape_szt()));
}

namespace {
OpxCreator<MinOpx> minOpxCreator({Onnx::Operators::Min_6,
                                  Onnx::Operators::Min_8});
OpxCreator<MinGradOpx> minGradOpxCreator(Onnx::GradOperators::MinGrad);
} // namespace
