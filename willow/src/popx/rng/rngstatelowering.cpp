...
PriTask RngStateLowering::initRngStateTensor() {
  ...
  auto initRngStateTensorTask = [this]() {
    const auto *deviceInfo = irLowering.get().getDeviceInfo();

    SequenceMap seqs(graph.get());
    std::vector<size_t> combinedRngStateTensorShape =
        getCombinedRngStateTensorShape(deviceInfo->getTarget());
...
PriTask RngStateLowering::rngStateFromHost() {
  auto rngStateFromHostTask = [this]() {
    ...
    auto streamRngFromHost = graph.get().addHostToDeviceFIFO(
        "h2d_rngStateTensor",
        poplar::UNSIGNED_INT,
        getCombinedRngStateTensorSize(deviceInfo->getTarget()),
        poplar::ReplicatedStreamMode::REPLICATE);

    logging::devicex::debug(
        "Initializing RNG h2d with RNG size equal to {}.",
        getCombinedRngStateTensorSize(deviceInfo->getTarget()));
...
PriTask RngStateLowering::rngStateToHost() {
  auto rngStateToHostTask = [this]() {
    const auto &target = irLowering.get().getDeviceInfo()->getTarget();

    auto streamRngToHost =
        graph.get().addDeviceToHostFIFO("d2h_rngStateTensor",
                                        poplar::UNSIGNED_INT,
                                        getCombinedRngStateTensorSize(target));
