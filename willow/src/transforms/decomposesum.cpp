...
// This class encapsulates an input tensor of a sum and the priority
...
std::size_t DecomposeSum::id() { return typeid(DecomposeSum).hash_code(); }

std::vector<Op *>
DecomposeSum::getDecomposableSumOps(const Graph &graph) const {
  std::vector<Op *> decomposableSumOps;
  for (auto &id_op : graph.getOps()) {
    Op *op = id_op.second.get();
    if (op->isConvertibleTo<SumOp>() && op->input->tensors().size() > 2) {
      decomposableSumOps.push_back(op);
    }
  }

  return decomposableSumOps;
}

void DecomposeSum::applyAddOpAttributes(Op *) const {}

// A cycles-for-liveness optimization. Turn summations into
// a schedulable tree of additions such that partial values do not
// have to wait until the point of summation of all partials
...
bool DecomposeSum::apply(Graph &graph) const {
  ...
  auto sumOps = getDecomposableSumOps(graph);
  std::map<OpId, std::vector<Tensor *>> sumOrders;
  ...
  for (Op *sumOp : sumOps) {
    ...
    inputMergePriorities.reserve(sumOp->input->n());

    for (auto *t : sumOp->input->tensors()) {
    ...
    // 2) Set the order in which to sum the partials according to their
    //    merge priorities.
    ...
    sumOrders[sumOp->id] = partialsSumOrder;
  }

  for (Op *sumOp : sumOps) {
    auto partialsSumOrder       = sumOrders[sumOp->id];
    const std::size_t nPartials = partialsSumOrder.size();
    ...
    sumOp->disconnectAllInputs();
    sumOp->disconnectAllOutputs();
    graph.eraseOp(sumOp->id);

    // Now replace with a series of Adds

    // Create InitOp to produce the initial partial input to the
    // addition tree. It inherits settings from the producer of the first
    // sum partial if available
    Op::Settings initSettings = [&]() {
      if (partialsSumOrder.front()->hasProducer()) {
        auto firstPartialProducer = partialsSumOrder.front()->getProducer();
        auto settings             = firstPartialProducer->getOutSettings(
            firstPartialProducer->output->indices(partialsSumOrder.front())
                .front());
        settings.name = sum->id + "_InitOp";
        return settings;
      } else {
        return Op::Settings{graph, sum->id + "_InitOp"};
      }
    }();
    ...
    TensorId sumInit = sum->id + "_init";
    initOp->createAndConnectOutTensor(InitOp::getOutIndex(), sumInit);
    ...
    for (std::size_t i = 0; i < nPartials; i++) {
      ...
      std::unique_ptr<popart::Op> add = OpManager::createOp(
          Domain::ai_onnx,
          "Add",
      ...
      OpId opId = graph.moveIntoGraph(std::move(add));
      Op *op    = graph.getOps()[opId].get();
      op->connectInTensor(AddOp::getArg0InIndex(), addLhsId);
      op->connectInTensor(AddOp::getArg1InIndex(), t->id);
      if (i == nPartials - 1) {
        // The final summed tensor - it already exists in the Ir
        op->connectOutTensor(AddOp::getOutIndex(), sum->id);
      } else {
        TensorId partSummedId = sum->id + "_" + std::to_string(i);
        op->createAndConnectOutTensor(AddOp::getOutIndex(), partSummedId);
        // For the next Add
        addLhsId = partSummedId;
      }

      // The sumInit accumulator needs the same tensor layout as the partials.
      // Allow the AddOp to propagate the tensor layout from the partials
...
bool init = Transform::registerTransform(new DecomposeSum());
...
uint64_t earliestForTensor(const PoprithmsTransitiveClosure &tc,
  ...
  // If the Tensor doesn't have a producer then it's earliest position would be
...
TensorMergePriority TensorMergePriority::of(Tensor *t,
  ...
  if (t == nullptr) {
    throw internal_error("decomposesum: TensorMergePriority::of: Provided "
                         "tensor is null.");
  }
  ...
  if (t->hasProducer()) {
    ...
    Priority p{op->hasPipelineStage() ? op->getPipelineStage()
                                      : unusedPipelineStage,
...
/*
  Note: This comment is specifically for decomposing grad sums. The above has
  been refactored to provide an all-sum generalisation. However the correctness
  concerns in [comment:all-sums] have been addressed by pre-computing all
  "earliests".
  TODO(T66608)
