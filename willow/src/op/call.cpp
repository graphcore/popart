      ...
      if (!hasGraph) {
        ...
        if (info.hasInputIds()) {
          ...
          for (int i = 0; i < callee.input_size(); i++) {
            ...
            auto scopedId = addScope(*calleeGraph, callee.input(i).name());
        ...
        for (auto implicitTensorId : implicitTensorIds) {
          ...
          auto calleeScopedId = addScope(*calleeGraph, implicitTensorId);
        ...
        for (auto &output : callee.output()) {
          auto scopedId = addScope(*calleeGraph, output.name());
