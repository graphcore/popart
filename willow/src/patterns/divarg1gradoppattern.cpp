...
bool DivArg1GradOpPattern::apply(Op *op) const {
  ...
  square->createAndConnectOutTensor(0, createImtermediateTensorId(grad_in->id));
  square->outInfo(0) = square->inInfo(0);

  mul->connectInTensor(0, grad_in->id);
  mul->connectInTensor(1, fwd_in0->id);
  mul->createAndConnectOutTensor(0, createImtermediateTensorId(grad_in->id));
  ...
  div->createAndConnectOutTensor(0, createImtermediateTensorId(grad_in->id));
  div->outInfo(0) = npOut(div->inInfo(0), div->inInfo(1));

  negate->connectInTensor(0, div->outTensor(0)->id);
  negate->createAndConnectOutTensor(0, createImtermediateTensorId(grad_in->id));
