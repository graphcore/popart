...
loss scaling can also improve numerical stability of the gradient
calculations. If scaledOptimizerState is enabled then the the
lossScaling will not be removed before updating the optimizer state.
...
Parameter ``scaledOptimizerState``:
    Experimental Option. Does not remove lossScaling before updating
    the optimizer state. This should have no effect on the update
    equation. However, it does ensure a more numerically stable
    implementation when accl1_type is set to DataType::FLOAT16. Note:
    When loading a model that includes initialised optimizer state,
    ensure that accl1 and accl2 are scaled by lossScaling and
    lossScaling^2 respectively.)doc";
...
Calculate the most likely \p topPaths labels and their probabilities
given the input \p logProbs with lengths \p dataLengths.

Parameter ``args``:
    Vector of input tensor ids. These are [logProbs, dataLengths],
...
Returns:
    ...
    labelLengths, decodedLabels], where labelProbs is of shape
    [batchSize, topPaths], labelLengths is of shape [batchSize,
    topPaths], and decodedLabels is of shape [batchSize, topPaths,
    maxTime].)doc";
...
Unpack packed sequences of data according to lengths and offsets
tensors, and call the callback on the unpacked sequences.

Parameter ``args``:
    Input tensors.

Parameter ``maxSequenceLengths``:
    The maximum length of a sequence in each of the data inputs.

Parameter ``resultSize``:
    The size of the first dimension of the result tensor.

Parameter ``callbackBatchSize``:
    The number of batches to pass to the callback.
...
backwards graph are available as either inputs or outputs of the
forward graph or gradients of outputs of the forward graph.
...
gradients of outputs of the forward graph. Strategies may expose
tensors that would otherwise have been internal to the forward graph
as outputs of said forward graph.)doc";
...
tensors that are neither inputs or outputs in the forward graph, add
them as outputs to the forward graph.

NOTE: This strategy is not guaranteed to work for all circumstances.
In particular, it is unable to deal with subgraphs of IfOp. Using this
...
static const char *__doc_popart_AutodiffStitchStrategy_RecomputeAllNonInputs =
    R"doc(Recompute any backward graph inputs associated with non-gradient
forward graph tensors that are not inputs in the forward graph.)doc";

static const char *__doc_popart_AutodiffStitchStrategy_RecomputeMinimal =
    R"doc(Recompute any backward graph inputs associated with non-gradient
forward graph tensors that are neither inputs nor outputs in the
forward graph.)doc";

static const char *__doc_popart_AutodiffStitchStrategy_SafeAddFwdOutputs =
    R"doc(Like `AddFwdOutputs` except that those backward graph inputs that
can't be stitched with `AddFwdOutputs` (that is, by adding outputs to
the forward graph) are stitched using the `RecomputeMinimal` strategy
instead. This means that this is a safe strategy to use as an
`Autodiff` default.)doc";
...
range of the tracked gradient tensor elements, in the range [0, 1].
`0` represents the smallest representable value, and `1` the maximum.
This is the single bin edge of the histogram that is an input to the
loss scale updater algorithm.)doc";

static const char *__doc_popart_AutomaticLossScalingSettings_enabled =
    R"doc(If true, keep track of the distribution of gradient tensor elements
over the floating point range. Adjust the value loss scaling tensor
...
static const char
    *__doc_popart_AutomaticLossScalingSettings_thresholdUpperCountProportion =
        R"doc(The proportion of the elements in the upper bin above which the loss
scale is increased, and below which the loss scale is decreased.
Should be in the range [0, 1].)doc";
...
will be collected. If unset, the gradients of all tensors produced by
a default operations (matmul, conv) will be used.)doc";
...
static const char *__doc_popart_Builder_addInputTensor_4 =
    R"doc(Add a new input tensor to the model (with data type and shape, with \p
TensorInfo and additional settings for \p TileSet and \p
ExchangeStrategy).
...
static const char *__doc_popart_CommGroupType_Consecutive =
    R"doc(Groups are consecutive in replica. If there are N replicas denoted {0,
... N-1} and group size is k, then there are N/k groups of size k: {0,
1, ... k-1}, {k, ... 2k-1} ... {N-k-1, ... N-1})doc";

static const char *__doc_popart_CommGroupType_N =
    R"doc(Groups are sliced orthogonal to the replica ordering. If there are N
replicas denoted {0, ... N-1} and group size is k, then there are m =
...
static const char *__doc_popart_ExchangeBaseOp_getExchangeDescriptor =
    R"doc(Return the exchange descriptor at index A \p MultiExchangeOp can
contain multiple descriptors, while \p RemoteLoad/Store and \p
HostLoad/Store contain one each.

Parameter ``index``:
    Index of the exchange descriptor to return.
...
Parameter ``direction``:
    \p Load (from host) or \p Store (to host)

Parameter ``id``:
    Host stream tensor ID
...
Parameter ``numInputs``:
    Number of tensor inputs expected

Parameter ``numOutputs``:
    Number of tensor outputs expected)doc";
...
static const char *__doc_popart_ExchangeDescriptor_getResourceId =
    R"doc(Get an identifier representing which resource (landing pad tensor)
this exchange will be using
...
executing ONNX graphs on IPU hardware, without any automatic
differentiation (backpropagation) or optimization.)doc";
...
static const char *__doc_popart_InputSettings =
    R"doc(A class that describes the #TileSet and #ExchangeStrategy used for an
input tensor.)doc";
...
static const char *__doc_popart_Ir_anchorRemap =
    R"doc(Map between actual tensor that provides the expected data and user-
defined anchor tensor, based on how the graph was transformed and the
anchor return type)doc";
...
The OpIds and TensorIds will differ between the original and the
cloned graph. Hence a map between the old OpId and cloned OpId will be
returned. The new graph can be obtained by ir.getGraph(newGraphId);

Parameter ``originalGraphId``:
    The id of the graph to clone

Parameter ``newGraphId``:
    The id of the cloned graph
...
static const char *__doc_popart_Ir_getHostLoadTensors =
    R"doc(The original input tensor ID (used to identify streams) and the
tensors produced by associated HostLoadOp.)doc";

static const char *__doc_popart_Ir_getHostStoreTensors =
    R"doc(The original anchor tensor ID (used to identify streams) and the
tensors consumed by associated HostStoreOp.)doc";
...
static const char *__doc_popart_Ir_hasOnnxModel =
    R"doc(Check if there's an ONNX model in the IR. This is true if the IR has
been created from an ONNX model or using the Builder.
...
static const char *__doc_popart_Ir_setIsPrepared =
    R"doc(Marks the Ir as "prepared". This means the Ir is now ready to be
lowered. Failing to do this before lowering the Ir will result in an
error.)doc";
...
static const char *__doc_popart_Op_growAliasModel =
    R"doc(For certain tasks which involve analysing how Tensors alias each
other, such as inplacing, a poprithms::memory::inplace::Graph, which
corresponds to this Op's Graph, is constructed. The poprithms Graph
can then be queried for aliasing information, and can have algorithms
run on it.
...
poprithms::memory::inplace::Op equivalents into the poprithms Graph,
which is the container *popAliaser*.

\pre All input tensors of this `Op` have mappings in `aliasModel`
before the call to `aliasModel`. \post All output tensors of this `Op`
have mappings in `aliasModel` after to the call to `aliasModel`.
...
variant (that is, they have no "inplace" variants), and do no non-
trivial view-changing. Examples: LSTM, Conv, Matmul, SumReduce, etc.)doc";
...
static const char *__doc_popart_Op_inheritPlacementAttributes =
    R"doc(Helper function to set Op's placement attributes by inheriting them
from other ops in the graph. Attributes that are set include: -
Pipeline stage - Execution phase - Virtual graph ID - Batch serial
phase (optional)

Parameter ``inheritSerializations``:
    Set batch serialial phase or not.

Parameter ``aliasModel``:
    An AliasModel containing alias info for this op's graph.)doc";
...
static const char *__doc_popart_Op_isInplaceViewChange =
    R"doc(Is this op a view changing op? E.g. does it not modify it's input, and
is it an inplace op? Set at each op level. Examples: ReshapeInplaceOp,
...
static const char *__doc_popart_Op_isOutplaceViewChange =
    R"doc(Same as above for outplace (non-inplace) ops. Examples: ReshapeOp,
IdentityOp, TransposeOp
...
static const char *__doc_popart_Op_mapInplaceProposal =
    R"doc(Translate a PopART inplacing proposal, which replaces this non-inplace
Op with an inplace Op of type #inplaceId, into an AliasModel
equivalent.

Parameter ``aliasModel``:
    Contains the mapping between this Op's (PopART) Graph and the
    poprithms Graph.

Parameter ``inplaceId``:
    The OperatorIdentifier to translate to the AliasModel equivalent.

Returns:
    A tuple where the first element corresponds to an alias gate in
    the AliasModel and the second element is a input index.

This method is defined as a void method which sets a value passed by
reference, as opposed to a getter method, so that no poprithms headers
need to be included in this file.)doc";

static const char *__doc_popart_Op_mapInplaceProposalGate0 =
    R"doc(Set the Proposal to open the poprithms::AliasGate which corresponds to
this Op, at input index 0. For information on AliasGates and inplacing
...
- The end-user facing interface constructed by the user to describe
what kind of optimiser to use. - Then also used internally by the Ir
to grow the optimiser step for each weight. - Stores OptimizerValues
for optimizer parameters like learning rate, loss scaling, etc.

See also:
    OptimiserValue. - Optimizer stores the values for each weight -
    they can have different values. There is a "default" for all
    weights, then you can specify specific values for specific
    weights. This is encapsulated by an OptimizerValueMap, which is a
    sparse map from weight to value, with unspecified values implying
    the default.

See also:
    OptimizerValueMap. - At runtime, the user can dynamically update
    the Optimizer, e.g. by setting new OptimizerValues.
    validReplacement determines whether the new Optimizer is
    interchangable with the one the Ir was built for. For example,
    trying to replace an SGD Optimizer with an Adam Optimizer would
    throw.)doc";

static const char *__doc_popart_OptimizerReductionType =
    R"doc(Reduction mode when doing data-parallel training over replicated
graphs.

Depending on the optimizer used and its configuration, this option
describes how the reduction of gradients over replicas will occur. For
example, directly on the gradient, on the gradient accumulator, or on
the momentum. See the documentation of individual optimizers for more
information.)doc";
...
Specifically, The TensorId at index i will be the id of the input
tensor at InIndex i of the VarUpdateOp. If the input is an
OptimizerValue, if it is const, then "" will be returned, else the
relevant reservered prefix for that OptimizerValue will be used,
followed by the weight id. The prefixes are defined in
tensornames.hpp, for example
`reservedDefaultWeightDecayScaleFactor0Prefix` or
`reservedSpecificScaledLearningRate1Prefix` (note there are different
prefixes depending on if the weight has a specific or default value
for that OptimizerValue).)doc";
...
See the SGD notes in optimizer.hpp for a more detailed and
comprehensive derivation of the SGD optimizer step in PopART.)doc";

static const char *__doc_popart_SGDAccumulatorAndMomentum =
    R"doc(Strategy for implementing SGD with momentum and/or gradient
accumulation.)doc";

static const char *__doc_popart_SGDAccumulatorAndMomentum_Combined =
    R"doc(Implement SGD using a single tensor for the gradient accumulator
(accum) and momentum (accl) tensors.)doc";

static const char *__doc_popart_SGDAccumulatorAndMomentum_Separate =
    R"doc(Implement SGD using separate tensors for the gradient accumulator
(accum) and momentum (accl) tensors)doc";
...
Parameter ``sgdAccMm``:
    The implementation strategy to use when gradient accumulation
    and/or momentum are used, otherwise ignored.
...
Parameter ``accumType``:
    The DataType of the accum tensor, when gradient accumulation is
    used and sgdAccMm = SGDAccumulatorAndMomentum::Separate, otherwise
    ignored. Only FLOAT, FLOAT16 and UNDEFINED are supported. Defaults
    to UNDEFINED. If UNDEFINED, the same type as the weights will be
    used. If accumType is FLOAT16 and accl1Type is FLOAT, this
    parameter causes accum to be upcasted before being passed to the
    op that updates accl1.

Parameter ``accl1Type``:
    The DataType of the accl1 tensor, when gradient accumulation is
    used and sgdAccMm = SGDAccumulatorAndMomentum::Separate, otherwise
    ignored. Only FLOAT, FLOAT16 and UNDEFINED are supported. Defaults
    to UNDEFINED. If UNDEFINED, the same type as the weights will be
    used. If accumType is FLOAT16 and accl1Type is FLOAT, this
    parameter causes accum to be upcasted before being passed to the
    op that updates accl1.)doc";
...
static const char *__doc_popart_SGD_SGD_3 =
    R"doc(Default constructor Creates SGD with default scalars (equivalent to
getUnset<scalar>() methods), and other default parameters of main
constructor.)doc";

static const char *__doc_popart_SGD_SGD_4 =
    R"doc(Construct an SGD instance with default values.)doc";
...
The required compound scalar OptimizerValues for the VarUpdateOp wil
be computed and passed to the Op. See the SGD notes above this class
for how they are derived. Recall that if non-const, the VarUpdateOp
will take an input Tensor for the compound scalar.

The OptimizerReductionType of the Op is derived as follows: No
replication => None Replication, no grad acc => GradReduce
Replication, grad acc, SGD1 => AcclReduce Replication, grad acc, SGD2
=> AccumReduce See the SGD notes above this class for why this is.
...
An SGD1ComboOp will later be decomposed by SGD1Decompose pattern into
a series of Ops and Tensors that implement the SGD1 optimiser step.

See also:
    SGD1Decompose

An SGD12ComboOp will later be decomposed by SGD2Decompose pattern into
a series of Ops and Tensors that implement the SGD2 optimiser step.
...
Parameter ``params``:
    A parameter map where keys are one of `"learningRate"`,
    `"weightDecay"`, `"momentum"`, `"dampening"`, or
    `"velocityScaling"` and the map's values pairs of floats and
    booleans representing OptimizerValue constructor arguments. The
    map does not have to specify each hyper parameter as default
    values will be used where parameters are missing.)doc";
...
static const char *__doc_popart_SessionOptions_automaticLossScalingSettings =
    R"doc(Settings to enable and configure the automatic loss scaling behaviour
when training.)doc";
...
static const char *__doc_popart_SessionOptions_compilationProgressLogger =
    R"doc(Callback function used to to indicate PopART compilation progress. The
function is passed two integers. The first is the progress value and
the second is the maximum value for the progress.

The function should not block. All calls to the callback function will
be made from the main thread so blocking in the callback will block
compilation from progressing.

If this logger is not set then compilation progress will be printed on
the info channel.)doc";
...
static const char *__doc_popart_SessionOptions_enableSupportedDataTypeCasting =
    R"doc(If enabled, casts any tensor of unsupported data types to supported
data types when lowering to Poplar Currently, this implies casting:
INT64 -> INT32 UINT64 -> UINT32 The cast will error for incompatible
data types and over/underflows, and inform on narrowing casts)doc";
...
static const char
    *__doc_popart_SessionOptions_transitiveClosureOptimizationThreshold =
        R"doc(The transitive closure optimization pass can significantly accelerate
the scheduler. It does not in general affect the final schedule
returned. It is run between initialization with Kahn's algorithms and
the shifting swaps. The transitive closure optimization pass is
O(nOps^2) and so should not be used for extremely large Graphs. If a
Graph is above the following threshold, the transitive closure
optimization pass is not run.)doc";
...
static const char *__doc_popart_Session_connectStream =
    R"doc(Connect a given poplar stream handle with a buffer to copy the memory
to or from IPU.)doc";
...
If the return value is false, you will be unable to obtain an instance
of TensorInfo using getInfo.)doc";
...
executing ONNX graphs on IPU hardware with training provided by
optimizing the specified loss tensor using the specified optimizer and
automatic differentiation (backpropagation))doc";
...
static const char *__doc_popart_VgidAndTileSetAndPTensorCmp = R"doc()doc";

static const char *__doc_popart_VgidAndTileSetAndPTensorCmp_operator_call =
    R"doc()doc";
...
Parameter ``sgdAccMm``:
    SGDAccumulatorAndMomentum reference.
